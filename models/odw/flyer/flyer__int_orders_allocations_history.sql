{{ config(
    ...
    unique_key='trading_session_date',
    incremental_strategy='delete+insert',
    on_schema_change='sync_all_columns',
    cluster_by=['trading_session_date'],
    tags=['hourly']
) }}
...
with destination_summary as (
    {% if is_incremental() -%}
    select trading_session_date, max(_created_at) as _created_at, max(_source_loaded_at) as _source_loaded_at
    from {{ this }}
    where 1 = 1
        -- Model start date. This applies for full-refresh.
        and trading_session_date >= '{{ start_date }}'
        {%- if is_incremental() or target.name not in ['prod'] %}
        -- Restrict lookback window if incremental or not prod.
        and trading_session_date >= current_date() - {{ lookback }}
        {%- endif %}
    group by all
    order by 1,2
    {% else -%}
    select null::date as trading_session_date
...
, date_spine as (
    select trading_session_date from source_summary group by all
    union
    select trading_session_date from destination_summary group by all
)
...
, dates_to_refresh as (
    ...
    left join source_summary s
        on a.trading_session_date = s.trading_session_date
    left join destination_summary d
        on a.trading_session_date = d.trading_session_date
    where 1 = 1
        and (
...
where 1 = 1
    and a.trading_session_date in (select distinct trading_session_date from dates_to_refresh)
