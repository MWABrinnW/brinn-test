with cte_flyer as (
    ...
    where is_head_for_day = 1 and custodian ilike any ('schwab' , 'fidelity')
    group by 1 , 2
)
...
, cte_allocations as (
    ...
    select
        member_account         as account_number
        , trading_session_date as trade_date
    from {{ ref('flyer__int_orders_allocations') }}
    group by 1 , 2
)
...
from {{ ref('bld_custodian_transactions') }} as a
...
where 1 = 1
    and a.buy_sell is not null
    and a.custodian in ('schwab' , 'fidelity')
