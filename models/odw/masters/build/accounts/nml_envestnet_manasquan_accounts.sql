...
{%-
    set src_models = [
          'envestnet_manasquan__stg_account_master'
    ]
-%}
...
select
    ...
    , a.account_id::text(500)                                                 as pms_account_id
    ...
    , a.customer_id::text(100)                                                as pms_client_id
    , a.customer_name::text(500)                                              as pms_client_name
    , iff(a.close_date is null , 1 , 0)::int                                  as pms_is_active
    ...
    , a.product_name::text(500)                                               as pms_model_investment_strategy
    , null::text(500)                                                         as pms_aum_classification
    ...
    -- COALESCE ---------------------------------------------------------------
    {{ select_nml_account_coalesce('envestnet__manasquan') }}
    ...
    || coalesce(case
        when pms_client_name = 'Closed Accounts'
            then 'Closed Accounts household;'
    end , '')
        as excluded_reasons
    , case
    ...
    , object_construct_keep_null(
        'join_sf1_eff_date' , iff(sf1.system_instance is not null , 1 , 0)
        , 'join_sf2_is_head' , iff(sf2.system_instance is not null , 1 , 0)
        , 'join_map_aum_glo' , iff(map_cus_glo.source_value is not null , 1 , 0)
...
left join {{ ref('aux__stg_masters_mappings') }} as map_aum_glo
    ...
    and a.effective_date between coalesce(map_aum_glo.start_date , a.effective_date) and coalesce(
