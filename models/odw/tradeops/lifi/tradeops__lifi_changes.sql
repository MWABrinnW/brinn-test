with cte_links as (
    select
        ...
        , ca.total_value
            as account_value
        , row_number()
            over (partition by al.effective_date , al.custodian , al.account_number order by al._source_loaded_at desc)
            as rn
    from {{ ref('custodian_account_links') }} as al
...
, cte_orion as (
    ...
    qualify
        row_number() over (partition by effective_date , custodian , account_number order by _source_loaded_at desc)
        = 1
)
...
, cte_final as (
    select
        ...
        , iff(
            sf.subadvisor_c in ('001C000001aKJMGIA4')
            and sf.custodian not in ('schwab' , 'fidelity') , 1 , 0
        )                                                        as is_subadvisor_lifi
        ...
        , case
            ...
            -- this category is expected to be empty
            when is_lifi = 0 and is_salesforce_lifi = 0
