with black_diamond_relationship as (
    ...
    where true
        and effective_date in (select distinct bb.cash_available_date from {{ ref('black_diamond_uhnw__base_bills') }} as bb)
...
saleforce_salentica as (
    ...
    where true
        and date(sfba.effective_at) in (
            select distinct bb.cash_available_date from {{ ref('black_diamond_uhnw__base_bills') }} as bb
        )
        and sfs.is_latest = 1
...
select
    ...
    , ba.aum_aua_ro::text(200)                                                   as aum_classification_status
    ...
    , '6640'::text(200)                                                          as coa_segment_3_accounting_id
