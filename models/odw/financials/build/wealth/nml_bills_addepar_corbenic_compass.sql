select
    ...
    -- [location]
    , coalesce(acc.household_location_code , acc2.household_location_code)::text(500) as client_location_code

    -- [invoice]
    , b.billing_id::text(500)                                                         as invoice_number_source
    ...
    , b.billing_bill_to_account_number::text(500)                                     as billing_account_number
    , b.entity_id::text(500)                                                          as account_id_pms
    , a.top_level_owner::text(500)                                                    as registrant_name
    , b.name::text(500)                                                               as account_name
    , coalesce(acc.registration_type , acc2.registration_type)::text(500)             as type_of_account
    , a.top_level_owner_entity_id::text(500)                                          as client_id_pms
    , coalesce(acc.aum_classification , acc2.aum_classification)::text(500)           as aum_classification_status
    , coalesce(acc.investment_strategy , acc2.investment_strategy)::text(500)         as model_investment_strategy
    -- confirm this matches FA Master
    , coalesce(acc.custodian_key , acc2.custodian_key)::text(500)                     as custodian
    , null::text(500)                                                                 as billing_custodian
    , null::text(500)                                                                 as partner_firm
    , null::text(500)                                                                 as partner_firm_original

    -- [advisor]
    , null::varchar(500)                                                              as advisor_source
    -- Historical client manager (from compass account object, historical records)
    , acc.client_manager::varchar(500)                                                as advisor_original
    ...
    , acc2.client_manager::varchar(500)                                               as advisor_primary
    -- Current associate id (from compass account object, is_head)
    , acc2.employee_number::varchar(500)                                              as associate_id_primary
    , 'W-2'::varchar(500)                                                             as advisor_type

    -- [assets and fees]
    -- fee type requires null handling, deteremines revenue category
    , lower(coalesce(trim(b.billing_fee_type) , 'management fee'))::text(500)         as fee_type
    ...
    end::text(500)                                                                    as billing_style
    -- sourced from "aux__stg_financials_fee_type" if not hardcoded
    , lower(coalesce(b.billing_schedule_interval , 'monthly'))::text(500)             as billing_frequency
    , a.billing_payment_method::text(500)                                             as billing_method
    ...
    , '1197'::text(500)                                                               as coa_segment_3_accounting_id
    ...
    -- [crm]
    , coalesce(acc.system_name , acc2.system_name)::text(500)                         as system_name_crm
    , coalesce(acc.system_instance , acc2.system_instance)::text(500)                 as system_instance_crm
    , coalesce(acc.system_key , acc2.system_key)::text(500)                           as system_key_crm
    , coalesce(acc.id , acc2.id)::text(500)                                           as account_id_crm
    , coalesce(acc.client_id , acc2.client_id)::text(500)                             as client_id_crm
    , coalesce(acc.client_id , acc2.client_id)::text(500)                             as client_id_original_crm
    , coalesce(acc.unique_identifier , acc2.unique_identifier)::text(500)             as client_id_unique_compass
    , coalesce(acc.household_name , acc2.household_name)::text(500)                   as client_name
    , coalesce(acc.household_name , acc2.household_name)::text(500)                   as client_name_original_crm
    , coalesce(acc.household_lead_source , acc2.household_lead_source)::text(500)     as client_lead_source
    ...
    -- [referential]
    , concat(b.billing_id , '-' , b.account_number)::varchar(500)                     as _trans_key
    , b._created_at::timestamp_ntz(9)                                                 as _source_loaded_at
    , b._source_file::varchar(500)                                                    as _source_file
    , null::varchar(500)                                                              as _box_file_id
    -- [extra field]
