...
, cte_normalize as (
    ...
    left join {{ ref('aux__stg_financials_fee_type') }} as ovrd_fee_type
        ...
        and nml.fee_type = ovrd_fee_type.fee_type
)
...
select
    ...
    , case
        when nml.billing_frequency ilike 'Quarterly' and nml.is_mid_cycle_invoice = 0
            then
                case
                    when nml.billing_style ilike 'Advance'
                        then
                            last_day(dateadd('Quarter' , 1 , nml.invoice_date) , 'Quarter')
                    when nml.billing_style ilike 'Arrears' then
                        last_day(dateadd('Quarter' , 0 , nml.invoice_date) , 'Quarter')
                end
        when nml.billing_frequency ilike 'Quarterly' and nml.is_mid_cycle_invoice = 1
            then
                case
                    when nml.billing_style ilike 'Advance'
                        then
        ...
        when nml.billing_frequency ilike 'Monthly' then
            case
                when nml.billing_style ilike 'Advance'
                    then
                        last_day(dateadd('Month' , 1 , nml.invoice_date) , 'Month')
                when nml.billing_style ilike 'Arrears' then
                    last_day(dateadd('Month' , 0 , nml.invoice_date) , 'Month')
            end
    end::date
    ...
    , nml.is_mid_cycle_invoice::int                                                                      as is_mid_cycle_invoice
    ...
    , initcap(nml.client_key_tags_crm::varchar(200))                                                     as client_key_tags_crm
    ...
    , nml._invoice_key::varchar(200)                                                                     as _invoice_key
...
left join {{ ref('bld_associates') }} as ass
    ...
    and (ass.effective_at::date) = coalesce(nml.invoice_date , revenue_period_end_date)
...
{% if target.name == 'prod' %}
        and nml.system_key in ('addepar__corbenic', 'salesforce__compass')
    {% endif %}
