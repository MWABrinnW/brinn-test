...
with cte_associates as (
    ...
    where 1 = 1
        {% if is_incremental() -%}
            and effective_at::date > (select max(t.effective_at::date) from {{ this }} as t)
        {% endif -%}
)
...
from cte_associates as e
left join cte_ad_users as ad
