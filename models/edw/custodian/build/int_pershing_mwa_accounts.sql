-- depends_on: {{ source('pershing_mwa', 'accf') }}
-- depends_on: {{ source('pershing_mwa', 'acct') }}
-- depends_on: {{ ref('pershing_mwa__accf_a_main_account_information') }}
-- depends_on: {{ ref('pershing_mwa__accf_b_main_account_information') }}
-- depends_on: {{ ref('pershing_mwa__acct_a_main_account_information') }}
-- depends_on: {{ ref('pershing_mwa__acct_b_main_account_information') }}
-- depends_on: {{ ref('pershing_mwa__hldr_j_individual_entity_client_common_information') }}
-- depends_on: {{ ref('pershing_mwa__accf_c_main_account_information_address_1_and_2') }}
-- depends_on: {{ ref('pershing_mwa__acct_c_main_account_information_address_1_and_2') }}
...
(
    ...
    where true
        and effective_date >= (select min(effective_date) from pershing_mwa__accf_a_main_account_information)
)
...
(
    select
        ...
        ,f._source_loaded_at
        ,f._source_file
    from {{ ref('pershing_mwa__accf_a_main_account_information') }} f
    left join {{ ref('pershing_mwa__accf_b_main_account_information') }} b
        on f.effective_date = b.effective_date
        and f.account_number = b.account_number
    left join {{ ref('pershing_mwa__hldr_j_individual_entity_client_common_information') }} j
        on f.effective_date = j.effective_date
        and f.account_number = j.account_number
        and j.client_type = 'AH'
    left join {{ ref('pershing_mwa__accf_c_main_account_information_address_1_and_2') }} maic
...
(
    select
        ...
        ,f._source_loaded_at
        ,f._source_file
    from {{ ref('pershing_mwa__acct_a_main_account_information') }} f
    left join {{ ref('pershing_mwa__acct_b_main_account_information') }} b
        on f.effective_date = b.effective_date
        and f.account_number = b.account_number
    left join {{ ref('pershing_mwa__hldr_j_individual_entity_client_common_information') }} j
        on f.effective_date = j.effective_date
        and f.account_number = j.account_number
        and j.client_type = 'AH'
    left join {{ ref('pershing_mwa__acct_c_main_account_information_address_1_and_2') }} maic
        on f.effective_date = maic.effective_date
        and f.account_number = maic.account_number
    join cte_date_index dt
