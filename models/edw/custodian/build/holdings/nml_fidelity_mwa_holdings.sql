with cte_cost_basis as (
    ...
    from {{ ref('fidelity_mwa_history__vw_tlaopen_tax_accounting') }}
    where 1 = 1
...
left join {{ ref('fidelity_mwa_history__vw_secmast_1_security') }} as s1
    on p.effective_date = s1.effective_date
    and p.cusip = s1.cusip
left join {{ ref('fidelity_mwa_history__vw_secmast_1_security') }} as s2
...
left join {{ ref('int_fidelity_mwa_account_sweep_fund') }} as sweep
