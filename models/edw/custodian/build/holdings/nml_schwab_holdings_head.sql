with dates as (
    select coalesce(
        ...
        , (select max(effective_date) from {{ ref('schwab__base_positions') }})
...
left join {{ ref('schwab__base_cost_basis') }} as pcb
    ...
    and pcb.effective_date = (select max(effective_date) from dates)
...
where true
    and p.effective_date = (select max(effective_date) from dates)
    and p.rn = 1
