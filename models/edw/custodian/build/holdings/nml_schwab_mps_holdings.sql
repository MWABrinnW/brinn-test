select
  ...
  , p.market_value_settled_unsettled::decimal(15, 2)    as market_value
  , p.quantity_settled_unsettled::decimal(19, 9)        as units_shares
  ...
  , pcb.cost_basis_unamortized::decimal(19,9)           as cost_basis
...
from {{ ref('schwab_mps_history__base_rps_positions') }} p
left join {{ ref('schwab_mps_history__base_positions_cost_basis') }} pcb
    on p.effective_date = pcb.effective_date
    and p.master_account_number = pcb.master_account_number
...
qualify row_number() over(partition by p.effective_date, p.account_number, p.cusip order by nvl(pcb.cost_basis_unamortized,0) desc) = 1
