select
    ...
    , t.key_code::text(500)                   as transaction_type_1_source_code
    , t.transaction_type_mnemonic::text(500)  as transaction_type_2_source_code
    , null::text(500)                         as transaction_type_3_source_code
    , null::text(500)                         as transaction_type_4_source_code
    , null::text(500)                         as transaction_type_5_source_code
    -------------------------------------------------------------------------------------------
    ...
    , case
        ...
        when t.buy_sell_code = 'S'
            then 'sell'
        else t.buy_sell_code
    end::text(500)                            as buy_sell
    , case
        when t.bookkeeping_amount > 0
            then 'debit'
        else 'credit'
    end::text(500)                            as debit_credit_indicator
    ...
    , (
        t.trade_date
        || '_'
        || t.bkpg_reference_number
    )::text(500
    )                                         as transaction_id_source
    , array_to_string(
        array_construct_compact(
            ...
            , t.bkpg_reference_number
            , t.key_code
            , t.transaction_type_mnemonic
            , t.bookkeeping_quantity
            , t.bookkeeping_amount
            , t.bookkeeping_market_value
            , t.bkpg_description_line_1
        ) , '_'
    )                                         as transaction_id
...
from {{ ref('fidelity_baystate_history__vw_actvyd_activity') }} as t
left join {{ ref('custodian_accounts') }} as a
    on
...
-- Security attributes
left join {{ ref('fidelity_baystate_history__vw_secmast_1_security') }} as sec
    on
