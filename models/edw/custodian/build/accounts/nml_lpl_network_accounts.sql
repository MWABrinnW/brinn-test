select
    ...
    , a.subscriber_id::varchar(500)                     as custodian_link
    , 'subscriber_id'::varchar(500)                     as custodian_link_detail
    , a.rep_id::varchar(500)                            as rep_link
    , 'rep_id'::varchar(500)                            as rep_link_detail
    , ar.normalized::varchar(500)                       as account_type
    , ar.definition::varchar(500)                       as account_type_source_definition
    , a.institution_type::varchar(500)                  as account_type_source_code
    , a.open_date::date                                 as opened_date
    , a.account_name::varchar(500)                      as account_title
    , c.first_name::varchar(500)                        as first_name
    , c.middle_name::varchar(500)                       as middle_name
    , c.last_name::varchar(500)                         as last_name
    ...
    , trim(concat(
        coalesce(c.address_1 , '')
        , coalesce(c.address_2 , ' ')
        , coalesce(c.address_3 , ' ')
...
from {{ ref('lpl_network__base_accounts') }} as a
...
left join {{ ref('lpl_network__base_clients') }} as c
...
where true
    and a.firm_source = 'network'
qualify
    row_number()
        over (
            partition by a.effective_date , a.custodian , a.firm_source , a.account_number
            order by iff(a.open_date is not null , 0 , 1) asc
        )
    = 1
