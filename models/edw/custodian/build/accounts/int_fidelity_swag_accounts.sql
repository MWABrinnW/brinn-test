...
, source_summary as (
    {%- set src_model = 'fidelity_swag_history__vw_nabase_101_account' %}
...
, cte_account as (
  select
    ...
    from {{ ref('fidelity_swag_history__vw_raw_nabase_101_account') }} a
    where true
...
, cte_business as (
  select
    ...
    from {{ ref('fidelity_swag_history__vw_nabase_102_business') }}
    where true
      and effective_date in (select distinct effective_date from dates_to_refresh)
      -- Offer the snowflake query optimizer a chance to prune the query early
      -- if there are no dates to refresh.
...
, cte_person as (
  select
    ...
    from {{ ref('fidelity_swag_history__vw_nabase_102_person') }}
    where true
      and effective_date in (select distinct effective_date from dates_to_refresh)
      -- Offer the snowflake query optimizer a chance to prune the query early
      -- if there are no dates to refresh.
