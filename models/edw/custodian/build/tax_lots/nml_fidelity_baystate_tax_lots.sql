...
from {{ ref('fidelity_baystate_history__vw_tlaopen_tax_accounting') }}  t
left join {{ ref('custodian_firms') }}                             cf
          on t.firm_source = cf.firm_source
left join {{ ref('fidelity_baystate_history__vw_secmast_1_security') }} s
...
left join {{ ref('int_fidelity_baystate_account_sweep_fund') }} sf
