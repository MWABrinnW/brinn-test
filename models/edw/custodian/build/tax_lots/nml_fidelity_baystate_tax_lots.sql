...
from {{ ref('fidelity_baystate_history__vw_tlaopen_tax_accounting') }} as t
left join {{ ref('custodian_firms') }} as cf
    on t.firm_source = cf.firm_source
left join {{ ref('fidelity_baystate_history__vw_secmast_1_security') }} as s
...
left join {{ ref('int_fidelity_baystate_account_sweep_fund') }} as sf
