...
select
  ...
  , case when effective_date = (select max(effective_date) from {{ source('fidelity_mwa', 'tlaopendelta') }}) then 1 else 0 end as is_current
  , effective_date
  , record_date
  , record_datetime
  , source_file
from {{ source('fidelity_mwa', 'tlaopendelta') }}
where 1 = 1
