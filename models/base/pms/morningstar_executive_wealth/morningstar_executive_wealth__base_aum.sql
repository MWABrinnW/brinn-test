select
    ...
    , {{ col_is_head(reference=source('morningstar_executive_wealth', 'aum'), reference_date_col='effective_at', source_date_col='effective_at') }}
    , {{ col_is_current(date_col='effective_at') }}
    , effective_at
    , _created_at
    , _source_file
from {{ source('morningstar_executive_wealth', 'aum') }}
