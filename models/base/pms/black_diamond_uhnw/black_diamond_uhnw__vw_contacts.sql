select
    ...
    , {{ col_is_head(reference=source('black_diamond_uhnw', 'relationship'), 
        reference_date_col='effective_date', source_date_col='a.effective_date') }}
    , {{ col_is_current(date_col='a.effective_date') }}
from {{ source('black_diamond_uhnw', 'relationship') }} as a
