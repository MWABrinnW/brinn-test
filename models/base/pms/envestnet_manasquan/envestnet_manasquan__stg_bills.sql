select
    ...
    -- obtains the first part of the file name which is the date of record, deduplicate files downstream
    , substring(_box_file_name , 1 , position('__' in _box_file_name))::text as _box_file_name_part_1
    , {{ col_is_head_with_partition(reference=src
        , partition_col = '_box_file_name_part_1'
