...
, cte_morningstar_aum as (
    select
        iff(
            client_name = 'Friends Cove Mutual Ins. Co.' and account_name = 'Equity Account'
            , '001050977010' , replace(account_number , '  ' , ' ')
        )                                                                                             as account_number
...
, cte_hfw_account_data as (
    select
        nullif(financial_account_number , 'NULL')            as financial_account_number
...
, cte_fa_base as (
    select
        replace(t.customeraccountnumber , '  ' , ' ') as financial_account_number
        , o.tpg_id_number                             as internal_financial_account_number
        , t.effective_date
    from cte_tpg_accounts as t
    ...
    select
        ...
        , o.tpg_id_number                      as internal_financial_account_number
        , m.effective_date
    from cte_morningstar_accounts as m
    left join cte_tpg_morningstar_overlap as o
        on m.account_number = o.morningstar_acct_number
...
, cte_joined as (
    select
        ...
        , case
            ...
            else hfw.internal_household_number
        end                                                                    as internal_household_number
        , case
        ...
        , case
            ...
            else hfw.financial_account_name
        end                                                                    as financial_account_name
        , case
            when olap.client_name is not null then t_acc.customername
            when t_acc.effective_date is not null then t_acc.customername
            when m_acc.effective_date is not null then m_acc.client_name
            else hfw.household_name
        ...
        , case
            ...
            else hfw.model_investment_strategy
        end                                                                    as model_investment_strategy
        , iff(
            hfw.effective_at is not null , coalesce(hfw.client_manager , m_aum.advisor_name)
            , null
        )                                                                      as client_manager
        , hfw.fee_schedule                                                     as fee_schedule
        , hfw.erisa                                                            as erisa
        , iff(
            hfw.effective_at is not null
            and (
                t_acc.effective_date is not null
                or m_acc.effective_date is not null
                or m_aum.effective_date is not null
            ) , hfw.account_active
            , 0
        )                                                                      as account_active
        , iff(
            hfw.effective_at is null , 'Data Aggregation / Reporting Only'
            , hfw.aum_classification_status
...
, cte_fa as (
    select
        ...
        , coalesce(sf.location_name , j.location_name)                   as location_name
