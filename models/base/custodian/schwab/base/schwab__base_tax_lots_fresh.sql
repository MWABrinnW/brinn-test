with fresh_data as (
    ...
    where 1 = 1
        and a.effective_date > (select max(effective_date) from {{ ref('schwab__base_tax_lots_history') }})
...
, fresh_data_with_link_detail as (
    select
        cl.firm_source
        , cf.firm
        , a.*
    from fresh_data as a
...
select
    ...
    , dense_rank() over (
        partition by a.effective_date , a.account_number , a.firm_source
...
from fresh_data_with_link_detail as a
