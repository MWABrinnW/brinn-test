...
select
    ...
    , nullif(trim(substring(a.content , 9 , 10)) , '')::date  as report_run_date
    , nullif(trim(substring(a.content , 20 , 10)) , '')::date as account_balance_as_of_date
    , substring(a.content , 31 , 8)                           as network_master
    , substring(a.content , 40 , 8)                           as fa_master
    , substring(a.content , 49 , 9)                           as household_id
    , nullif(trim(substring(a.content , 59 , 10)) , '')::date as activation_date
    , case
        when trim(substring(a.content , 70 , 10)) = '' then null
        else trim(substring(a.content , 70 , 10))
    end::date                                                 as termination_date
    , ltrim(substring(a.content , 81 , 8) , '0')              as account_number
    ...
    end::decimal(16 , 2)                                      as account_daily_balance
    , case
    ...
    end::decimal(16 , 2)                                      as household_ave_billable_assets
    , case
    ...
    end::decimal(16 , 2)                                      as fees_due_for_household
...
from {{ source('schwab', 'referral_fees_current_quarter') }} as a
...
select
    ...
    , ltrim(substring(a.content , 81 , 9) , '0')              as account_number
    , trim(substring(a.content , 91 , 30))                    as client_name
    , trim(substring(a.content , 135 , 10))                   as billed_not_billed
    , case
        when trim(substring(a.content , 122 , 12)) = '' then null
        else trim(substring(a.content , 122 , 12))
    end::decimal(14 , 2)                                      as account_daily_balance
    , case
        when trim(substring(a.content , 146 , 4)) = '' then null
        else trim(substring(a.content , 146 , 4))
    end::int                                                  as count_of_accounts_within_household
    , case
        when trim(substring(a.content , 151 , 12)) = '' then null
        else trim(substring(a.content , 151 , 12))
    end::decimal(14 , 2)                                      as household_ave_billable_assets
    , case
        when trim(substring(a.content , 164 , 5)) = '' then null
        else trim(substring(a.content , 164 , 5))
    end::decimal(5 , 4)                                       as blended_tiered_rate
    , case
        when trim(substring(a.content , 170 , 12)) = '' then null
        else trim(substring(a.content , 170 , 12))
