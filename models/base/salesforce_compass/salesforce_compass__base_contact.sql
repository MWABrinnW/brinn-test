select
  ...
  , {{ col_is_head(reference=source('salesforce_compass', 'contact'), source_date_col='a.effective_at', reference_date_col='effective_at') }}
  , case when b.rn = 1 then 1 else 0 end                        as is_latest
from {{ source('salesforce_compass', 'contact') }} a
left join (
    select
