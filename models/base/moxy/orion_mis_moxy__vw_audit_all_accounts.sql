...
select
  ...
  -- , case when record_datetime::timestamp = (select max(record_datetime::timestamp)) from {{ source('orion_mis_moxy', 'audit_all_accounts') }}) 
  --       then 1 else 0 end as is_current
  , record_datetime::timestamp                  as record_datetime
  , record_date::date                           as record_date
from {{ source('orion_mis_moxy', 'audit_all_accounts') }}
where record_datetime = (select max(record_datetime) from {{ source('orion_mis_moxy', 'audit_all_accounts') }})
