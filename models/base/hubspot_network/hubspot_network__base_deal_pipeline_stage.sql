select
  ...
  , a.json:PROBABILITY::text(500)        as probability
  , a.json:CLOSED_WON::text(500)         as closed_won
  ...
  , {{ col_is_head(reference=source('hubspot_network', 'deal_pipeline_stage'), source_date_col='a.effective_at', reference_date_col='effective_at') }}
  , case when b.rn = 1 then 1 else 0 end as is_latest
from {{ source('hubspot_network', 'deal_pipeline_stage') }} a
left join (
