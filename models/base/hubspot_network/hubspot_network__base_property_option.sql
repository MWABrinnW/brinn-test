select
  ...
  , a.json:PROPERTY_ID::text(500)        as property_id
  , a.json:VALUE::text(500)              as value
  , a.json:DISPLAY_ORDER::text(500)      as display_order
  , a.json:DOUBLE_DATA::text(500)        as double_data
  , a.json:HIDDEN::text(500)             as hidden
  , a.json:READ_ONLY::text(500)          as read_only
  , a.json:_FIVETRAN_SYNCED::text(500)   as _fivetran_synced

  , a.effective_at::timestamp            as effective_at
  , a._created_at::timestamp             as _created_at
  , {{ col_is_head(reference=source('hubspot_network', 'property_option'), source_date_col='a.effective_at', reference_date_col='effective_at') }}
  , case when b.rn = 1 then 1 else 0 end as is_latest
from {{ source('hubspot_network', 'property_option') }} a
left join (
