select
    a.json:STAGE_ID::text(500)           as stage_id
  ...
  , a.json:IS_CLOSED::text(500)          as is_closed
  , a.json:TICKET_STATE::text(500)       as ticket_state
  ...
  , {{ col_is_head(reference=source('hubspot_network', 'ticket_pipeline_stage'), source_date_col='a.effective_at', reference_date_col='effective_at') }}
  , case when b.rn = 1 then 1 else 0 end as is_latest
from {{ source('hubspot_network', 'ticket_pipeline_stage') }} a
left join (
