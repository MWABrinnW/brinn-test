select
  ...
  , a.json:ADDED_AT::text(500)           as added_at
  ...
  , {{ col_is_head(reference=source('hubspot_network', 'contact_list_member'), source_date_col='a.effective_at', reference_date_col='effective_at') }}
  , case when b.rn = 1 then 1 else 0 end as is_latest
from {{ source('hubspot_network', 'contact_list_member') }} a
left join (
