select
  ...
  , a.json:OBJECT_TYPE_ID::text(500)     as object_type_id
  ...
  , {{ col_is_head(reference=source('hubspot_network', 'ticket_pipeline'), source_date_col='a.effective_at', reference_date_col='effective_at') }}
  , case when b.rn = 1 then 1 else 0 end as is_latest
from {{ source('hubspot_network', 'ticket_pipeline') }} a
left join (
