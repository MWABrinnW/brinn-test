{% set src = source('emoney_mwa', 'medical_insurance_accounts') %}
select
    _data:"accountnumber"::text(200)                                        as account_number
    , _data:"clientid"::text(200)                                           as client_id
    , _data:"grouphealthplansponser"::text(200)                             as group_health_plan_sponser
    , try_to_number(_data:"annualpremium"::text , 28 , 10)::int             as annual_premium
    , try_to_number(_data:"benefitamount"::text , 28 , 10)::int             as benefit_amount
    ...
    , try_to_number(_data:"deductible"::text , 28 , 10)::int                as deductible
