select
  ...
  , a.json:VALUE::text(3800)                              as value
  , a.json:SYSTEM_MODSTAMP::timestamp_tz                  as system_modstamp
  , a.json:LANGUAGE::text(1000)                           as language
  ...
  , {{ col_is_head(
    reference=source('salesforce_mps', 'category_node_localization'),
    source_date_col='a.effective_at',
    reference_date_col='effective_at'
    ) }}
  , case when b.rn = 1 then 1 else 0 end                  as is_latest
from {{ source('salesforce_mps', 'category_node_localization') }} a
left join (
    select
        effective_at::date                                                            as effective_at
      , _created_at                                                                   as _created_at
      , row_number() over (partition by effective_at::date order by _created_at desc) as rn
    from {{ source('salesforce_mps', 'category_node_localization') }}
    group by 1, 2
)                                                                 b
