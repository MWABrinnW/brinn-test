with cte_fresh_dates as (
    ...
    where _effective_at > (select max(t._effective_at) from {{ ref('active_directory__rpt_users_history') }} as t)
    group by all
...
from cte_users as u
left join cte_users as m
    on u._effective_at = m._effective_at
    and u.manager = m.distinguishedname
where 1 = 1
    and u._effective_at::date in (select t.effective_date from cte_fresh_dates as t)
qualify
    row_number() over (
