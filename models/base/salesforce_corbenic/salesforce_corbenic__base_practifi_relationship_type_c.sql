select
    'salesforce_practifi'::text(200)                          as system_name
    ...
    , {{ col_is_head(reference=source('salesforce_corbenic', 'practifi_relationship_type_c') , 
        source_date_col='a.effective_at', reference_date_col='effective_at') }}
    , case when b.rn = 1 then 1 else 0 end                    as is_latest
from {{ source('salesforce_corbenic', 'practifi_relationship_type_c') }} as a
left join (
    select
