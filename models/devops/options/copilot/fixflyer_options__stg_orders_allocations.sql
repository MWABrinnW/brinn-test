select
  ...
  , a.json:order.fixedIncome.accruedInterestAmt::number(19, 6)                       as order_fixedincome_accrued_interest_amt
  ...
  , case
    when a._created_at || a._uri in
      (select max(_created_at || _uri) from {{ source('copilot', 'allocations') }} group by _uri)
    then 1
    else 0
    end::int                                                                         as is_head
...
from {{ source('copilot', 'allocations') }}                                                     a
     left join (
                   select
                     _uri
   ...
   , lateral flatten(input => a.json, path => 'memberList', outer => true, mode => 'array')     m
   , lateral flatten(input => a.json, path => 'order', outer => true, mode => 'array')          o
where 1=1
