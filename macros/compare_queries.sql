...
final as (
    ...
    select * exclude (in_a, in_b)
        , in_a::int                 as in_a
        , in_b::int                 as in_b
        , iff(in_a = in_b, 1, 0)    as is_match
    from all_records
    where 1 = 1
        -- Check if there are any mismatches. If so, return everything.
        and exists (
            select *
            from all_records
            where not (in_a and in_b)
        )
    order by {{ primary_key ~ ", " if primary_key is not none }} 1 , 3 desc
