{%- macro gtol_c_equities_and_option_product_type_order_information(src) -%}
...
, trim(nullif(substring(content, 217, 1), '0'))::varchar(1) as product_identifier_for_record_c
, trim(nullif(substring(content, 218, 1), '0'))::varchar(1) as complex_order_instruction_code
, trim(nullif(substring(content, 219, 1), '0'))::varchar(1) as do_not_increase_dni_quantity_indicator
, trim(nullif(substring(content, 220, 1), '0'))::varchar(1) as do_not_reduce_price_dnr_indicator
, trim(nullif(substring(content, 221, 1), '0'))::varchar(1) as option_openclose_code
, to_number(nullif(nullif(trim(substring(content, 222, 18)), '000000000000000000'), '')) / power(10, 09)::number as limit_price
, trim(nullif(substring(content, 240, 2), '00'))::varchar(2) as price_qualifier_code
, to_number(nullif(nullif(trim(substring(content, 242, 18)), '000000000000000000'), '')) / power(10, 09)::number as stop_price
, trim(nullif(substring(content, 260, 1), '0'))::varchar(1) as order_variance_type_code
, trim(nullif(substring(content, 261, 1), '0'))::varchar(1) as quote_preference_code
, trim(nullif(substring(content, 262, 12), '000000000000'))::varchar(12) as trailer_to_market
, trim(nullif(substring(content, 274, 40), '0000000000000000000000000000000000000000'))::varchar(40) as external_vendor_id
-- , trim(nullif(substring(content, 314, 353), ''))::varchar(353) as not_used_2
