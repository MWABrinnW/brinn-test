...
, signed_to_numeric(nullif(nullif(trim(substring(content, 21, 18)), '000000000000000000'), '')) / power(10, 09)::number as expanded_first_call_price_for_fixed_income_or_strike
, signed_to_numeric(nullif(nullif(trim(substring(content, 39, 18)), '000000000000000000'), '')) / power(10, 09)::number as expanded_first_par_call_price_for_fixed_income_or_exercise
, signed_to_numeric(nullif(nullif(trim(substring(content, 57, 18)), '000000000000000000'), '')) / power(10, 09)::number as expanded_put_price
, signed_to_numeric(nullif(nullif(trim(substring(content, 75, 18)), '000000000000000000'), '')) / power(10, 09)::number as expanded_second_premium_call_price
