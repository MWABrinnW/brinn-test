...
, {{target.database}}.{{target.schema}}.signed_to_numeric(nullif(nullif(trim(substring(content, 21, 7)), '0000000'), '')) / power(10, 03)::number as coupon_rate_for_fixed_income_securities_or_indicated
, {{target.database}}.{{target.schema}}.YYYYDDD_to_date(nullif(substring(content, 28, 7), '0000000'))::date as maturityoption_expire_date
, trim(nullif(substring(content, 35, 9), '000000000'))::varchar(9) as underlying_security_cusip
, {{target.database}}.{{target.schema}}.signed_to_numeric(nullif(nullif(trim(substring(content, 44, 9)), '000000000'), '')) / power(10, 04)::number as first_call_price_for_fixed_income_or_strike_price_for_option
, {{target.database}}.{{target.schema}}.signed_to_numeric(nullif(nullif(trim(substring(content, 53, 9)), '000000000'), '')) / power(10, 04)::number as first_par_call_price_for_fixed_income_or_units_for_option
...
, {{target.database}}.{{target.schema}}.YYYYDDD_to_date(nullif(substring(content, 79, 7), '0000000'))::date as next_par_call_date
, {{target.database}}.{{target.schema}}.YYYYDDD_to_date(nullif(substring(content, 86, 7), '0000000'))::date as prerefunded_date
, {{target.database}}.{{target.schema}}.YYYYDDD_to_date(nullif(substring(content, 93, 7), '0000000'))::date as next_premium_call_date
, {{target.database}}.{{target.schema}}.YYYYDDD_to_date(nullif(substring(content, 100, 7), '0000000'))::date as dated_date_for_fixed_income_or_ex_dividend_date_for_ccyyddd
, {{target.database}}.{{target.schema}}.YYYYDDD_to_date(nullif(substring(content, 107, 7), '0000000'))::date as first_coupon_for_fixed_income_or_payable_date_for_equity
