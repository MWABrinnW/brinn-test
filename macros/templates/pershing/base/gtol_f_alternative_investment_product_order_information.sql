{%- macro gtol_f_alternative_investment_product_order_information(src) -%}
...
, trim(nullif(substring(content, 217, 1), '0'))::varchar(1) as product_identifier_for_record_f
, trim(nullif(substring(content, 218, 2), '00'))::varchar(2) as settlement_location_code
, trim(nullif(substring(content, 220, 1), '0'))::varchar(1) as alternative_investment_suitability_attestation
, trim(nullif(substring(content, 221, 1), '0'))::varchar(1) as subscription_document_delivered
, trim(nullif(substring(content, 222, 1), '0'))::varchar(1) as subscription_document_not_delivered_reason_code
, trim(nullif(substring(content, 223, 1), '0'))::varchar(1) as reinvest_dividend_incomecapital_gains
, trim(nullif(substring(content, 224, 3), '000'))::varchar(3) as branch_override
, trim(nullif(substring(content, 227, 1), '0'))::varchar(1) as transaction_fee_designation
, iff(substring(content, 246, 1) = '-', -1, 1) * to_number(nullif(nullif(trim(substring(content, 228, 18)), '000000000000000000'), '')) / power(10, 02)::number as transaction_fee_amount
, trim(nullif(substring(content, 246, 1), '0'))::varchar(1) as transaction_fee_amount_sign
, trim(nullif(substring(content, 247, 40), '0000000000000000000000000000000000000000'))::varchar(40) as external_vendor_id
-- , trim(nullif(substring(content, 287, 380), ''))::varchar(380) as not_used_2
