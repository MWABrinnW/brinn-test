...
select
    ...
    , a.system_name                                                       as pms
    , a.system_instance                                                   as pms_instance_location
    , a.system_key                                                        as pms_key
    , a.firm_source                                                       as firm_source
    , upper(a.account_number)                                             as account_number_formatted
    , regexp_replace(
        ltrim(upper(replace(trim(a.account_number) , '-' , '')) , '0')::text(200)
        , '\\s{2,}' , ' '
    )                                                                     as account_number
    , regexp_replace(
        ltrim(upper(replace(trim(a.account_number) , '-' , '')) , '0')::text(200)
        , '\\s{2,}' , ' '
    )                                                                     as pms_account_number
    ...
    , case
        when a.system_key = 'black_diamond__houston'
            then a.manager
        when a.system_key = 'black_diamond__uhnw'
            then a.team
        {% if instance | lower == 'baystate' %}
    ...
    -- KEYS -------------------------------------------------------------------
    , concat(a.system_key , '__' , pms_account_name)                      as pms_key__account_name
    , concat(a.system_key , '__' , pms_account_number)                    as pms_key__account_number
    , concat(a.system_key , '__' , coalesce(pms_advisor , crm_advisor))   as pms_key__advisor
    ...
    , case when dedupe_pms_count > 1 then 1 else 0 end                    as has_dupes
    , case
...
{% endif %}
left join {{ ref('black_diamond_' ~ instance ~ '__base_relationships') }} as r
...
left join {{ ref('aux__stg_rules_overrides') }} as ovrd_pms_acc
    on ovrd_pms_acc.scope = 'pms_key__account_number'
    and pms_key__account_number = ovrd_pms_acc.scope_key
    and
...
left join {{ ref('aux__stg_rules_overrides') }} as ovrd_pms_adv
    on ovrd_pms_adv.scope = 'pms_key__advisor'
    and pms_key__advisor = ovrd_pms_adv.scope_key
    and
