{%- macro envestnet_hierarchy_branch(src) -%}
select
    ...
    , nullif(split_part(content , '|' , 2), '')::int           as branch_id
    , nullif(split_part(content , '|' , 3), '')::varchar(128)  as branch_name
    , nullif(split_part(content , '|' , 4), '')::varchar(10)   as branch_short_name
    , nullif(split_part(content , '|' , 5), '')::int           as enterprise_id
    , nullif(split_part(content , '|' , 6), '')::int           as firm_id
    , nullif(split_part(content , '|' , 7), '')::varchar(16)   as branch_code
    , nullif(split_part(content , '|' , 8), '')::varchar(16)   as manager_rep_code
