{%- macro envestnet_proposal_segment(src) -%}
select
    ...
    , nullif(split_part(content , '|' , 4), '')::int           as segment_id
    , nullif(split_part(content , '|' , 5), '')::int           as segment_period_from
    , nullif(split_part(content , '|' , 6), '')::int           as segment_period_to
